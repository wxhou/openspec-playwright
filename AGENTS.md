...
- 🟡 精准改动：只改必要的，每一行都应能直接追溯到用户的请求；不"改进"相邻的代码/注释/格式，不重构没坏的东西，无关的既有死代码提一下即可、不删；改完清理自己造成的垃圾（未使用的 import/export/prop/console.log 等；清理与重构分开提交）。匹配现有风格
...
- 不写只适配样例输入的逻辑、魔法数字（入常量并注释原因）、具体值断言（除非明确要求）；路径/换行用语言内建跨平台 API，比较前归一化 EOL
