...
// `ExecFileOptions` doesn't extend). Cast to add it back.
type ExecFileOpts = CommonOptions & {
...
export async function update(options: UpdateOptions) {
  ...
  if (options.cli !== false) {
    ...
    try {
      ...
      console.log(
        chalk.gray("  Run manually: npm install -g openspec-playwright@latest"),
      );
    }
    ...
    if (cliUpdated) {
      ...
      try {
        ...
        console.log(
          chalk.yellow(
            "  ⚠ Re-execution failed, continuing with current binary...",
          ),
        );
 ...
 */
async function checkVersionShadow(): Promise<void> {
  ...
  if (!publishedVersion || resolvedVersion === publishedVersion) return;
  ...
  console.log(
    chalk.gray(
      `    This update script resolved from: ${resolvedPath.replace(/[\\/]package\.json$/, "")}`,
    ),
  );
  console.log(
    chalk.gray(
      "    A devDependency in this project's package.json is shadowing the global CLI binary.",
    ),
  );
  console.log(
    chalk.gray(
      "    Fix: upgrade or remove it with: npm uninstall openspec-playwright",
    ),
  );
  console.log(
    chalk.gray(
