  ...
  for (const line of lines) {
    ...
    if (line.match(/^\| \//)) {
      ...
      if (pathMatch) {
        ...
        if (result) {
          const icon = result.status === "ok" ? "explored" : result.status === "error" ? "error" : "skipped";
...
function buildSummaryTable(results: RouteResult[]): string {
  ...
  for (const r of results) {
    const icon = r.status === "ok" ? "ok" : r.status === "error" ? "error" : "skipped";
...
function appendFailureSection(content: string, results: RouteResult[]): string {
  ...
  for (const f of failures) {
    const notes = f.errorMessage ?? "unknown";
    failureLines.push(`| ${f.path} | ${f.errorMessage ?? "unknown"} | |`);
  }
  failureLines.push("");
...
async function runWorker(
  ...
  try {
    ...
    for (const route of routes) {
      ...
      const icon = result.status === "ok" ? chalk.green("ok") : chalk.red("err");
      const pathDisplay = route.length > 40 ? route.slice(0, 40) + "..." : route;
      console.log(
        `  [worker ${workerId}] ${icon}  ${pathDisplay}`,
      );
    }
...
export async function explore(options: ExploreOptions) {
  ...
  const numWorkers = options.parallel ?? 4;
  ...
  if (parsed.routes.length === 0) {
    console.log(chalk.yellow("  No routes found in app-exploration.md\n"));
    return;
  }
  ...
  // 6. Write updated app-exploration.md
  ...
  writeFileSync(explorationPath, updatedContent);
  ...
  console.log(
    `  Routes explored: ${totalCount}  ` +
      ...
      "  " +
      (errorCount > 0 ? chalk.red(`error ${errorCount}`) : chalk.gray(`error ${errorCount}`)) +
