...
export async function init(options: InitOptions) {
  ...
  execCmd(
    'npx openspec --version 2>/dev/null || echo "not found"',
    "OpenSpec",
    true,
  );
  ...
  if (options.mcp !== false) {
    ...
    if (globalMcp["playwright"] || localMcp["playwright"]) {
      ...
      try {
        ...
        console.log(
          chalk.yellow("  ⚠ Failed to run claude mcp add. Run manually:"),
        );
        console.log(
          chalk.gray(
  ...
  console.log(
    chalk.gray(
      `  ${hasClaude ? "7." : "6."} Or: openspec-pw doctor to verify setup\n`,
    ),
  );
...
async function installSchema(projectRoot: string) {
  ...
  console.log(
    chalk.green("  ✓ Schema installed: openspec/schemas/playwright-e2e/"),
  );
}
