...
export function getLatestMcpVersion(): Promise<string | null> {
  return new Promise((resolve) => {
    exec(
      "npm show @playwright/mcp version --json",
      { timeout: 15000 },
      (err, stdout) => {
        if (err) {
          resolve(null);
...
function parseMcpReadme(
  ...
  if (tools.length === 0) {
    console.error(
      "[openspec-pw] Warning: @playwright/mcp README format changed, no tools parsed. Healer will use default tools.",
    );
  }
  return tools;
}
...
export async function fetchMcpTools(
  ...
  try {
    ...
    const tgzFiles = readdirSync(tmpDir).filter(
      (f) => f.startsWith("playwright-mcp-") && f.endsWith(".tgz"),
    );
    ...
    console.log(
      chalk.yellow("  ⚠ Failed to fetch MCP tools — using default set"),
    );
    return [];
  }
}
...
export async function syncMcpTools(
  ...
  if (!latestVersion) {
    if (verbose)
      console.log(
        chalk.yellow("  ⚠ Could not fetch latest @playwright/mcp version"),
      );
    return false;
  }
  ...
  if (verbose)
    console.log(
      chalk.blue(
        `  - Updating from ${storedVersion ?? "unknown"} → ${latestVersion}`,
      ),
    );
  ...
  if (verbose) {
    if (tools.length > 0) {
      console.log(
        chalk.green(
          `  ✓ Healer tools synced to ${latestVersion} (${tools.length} tools)`,
        ),
      );
    } else {
      console.log(
        chalk.green(
          `  ✓ Healer tools synced to ${latestVersion} (default set)`,
        ),
      );
    }
  }
