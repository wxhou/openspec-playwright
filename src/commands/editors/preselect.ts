...
const INTENT_FILES: Partial<Record<EditorId, string[]>> = {
  claude: ["CLAUDE.md"],
  cursor: [".cursorrules"],
...
export function intentFileEditors(projectRoot: string): EditorId[] {
  const hits: EditorId[] = [];
  for (const id of Object.keys(INTENT_FILES) as EditorId[]) {
