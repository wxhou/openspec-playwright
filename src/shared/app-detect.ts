...
function hasRunnableScript(pkg: PackageJson): boolean {
  const scripts = pkg.scripts ?? {};
  return Boolean(
    scripts["dev:all"] || scripts.dev || scripts.start || scripts.serve || scripts.preview,
  );
}

export function findNpmRoot(projectRoot: string, maxDepth = 5): string {
  function search(dir: string, depth: number): string | null {
    if (depth > maxDepth) return null;
    const pkgPath = join(dir, "package.json");
    if (existsSync(pkgPath)) {
      const pkg = readPackageJson(pkgPath);
      if (pkg && hasRunnableScript(pkg)) return dir;
    }
...
export function chooseDevScript(scripts: Record<string, string>): string | undefined {
  if (scripts["dev:all"]) return "dev:all";
...
function detectPortFromEnv(env: NodeJS.ProcessEnv): { port: number; source: string } | undefined {
  for (const key of ["PLAYWRIGHT_PORT", "E2E_PORT", "VITE_PORT", "PORT"]) {
    const value = env[key];
    if (!value) continue;
...
function detectPortFromEnvFiles(npmRoot: string): { port: number; source: string } | undefined {
  for (const file of [".env.local", ".env.development", ".env"]) {
    ...
    if (port) return { port, source: file };
  }
  return undefined;
}

function detectVitePort(npmRoot: string): { port: number; source: string } | undefined {
...
function detectSeedBaseUrl(projectRoot: string): { baseUrl: string; source: string } | undefined {
  const seedSpec = join(projectRoot, "tests", "playwright", "seed.spec.ts");
  if (!existsSync(seedSpec)) return undefined;
...
function dependencyExists(pkg: PackageJson, name: string): boolean {
  return Boolean(pkg.dependencies?.[name] || pkg.devDependencies?.[name]);
}

function frameworkDefaultPort(pkg: PackageJson, command = ""): { port: number; source: string } | undefined {
  if (command.includes("vite") || dependencyExists(pkg, "vite")) return { port: 5173, source: "vite default" };
  if (command.includes("astro") || dependencyExists(pkg, "astro")) return { port: 4321, source: "astro default" };
  if (command.includes("next") || dependencyExists(pkg, "next")) return { port: 3000, source: "next default" };
  if (command.includes("nuxt") || dependencyExists(pkg, "nuxt")) return { port: 3000, source: "nuxt default" };
  return undefined;
}
...
function findFrontendConfigFile(dir: string): string | null {
 ...
 */
function findFrontendInDirs(seeds: string[], budget: number, baseDir: string): string | null {
  let checked = 0;
  const relPosixFromBase = (dir: string): string => {
    const rel = relative(baseDir, dir);
    const posix = rel.split(sep).join("/");
    return posix === "" ? "." : posix;
  };
  const visit = (dir: string, remaining: number): string | null => {
    if (remaining < 0 || checked >= WORKSPACE_SCAN_MAX_MEMBERS) return null;
    const pkg = readPackageJson(join(dir, "package.json"));
    if (pkg) checked++;
    if (pkg !== null) {
      const dep = FRONTEND_FRAMEWORK_DEPS.find((name) => dependencyExists(pkg, name));
      if (dep) return `${relPosixFromBase(dir)} (${dep})`;
    }
    const config = findFrontendConfigFile(dir);
    if (config) return `${relPosixFromBase(dir)} (${config})`;
 ...
 */
export function explainFrontendSignal(projectRoot: string): string | null {
  const npmRoot = findNpmRoot(projectRoot);
  const pkg = readPackageJson(join(npmRoot, "package.json"));
  if (!pkg) return null;
