 ...
 */
function loadFromAppKnowledge(projectRoot: string): Partial<OllamaConfig> {
  ...
  if (!existsSync(appKnowledgePath)) return {};

  try {
    const content = readFileSync(appKnowledgePath, "utf-8");
    const match = content.match(
      /## Vision Check Config\s*\n([\s\S]*?)(?=\n##|\n---|\z)/,
    );
    if (!match) return {};
    ...
    if (enabledMatch) config.enabled = enabledMatch[1].trim() === "true";
