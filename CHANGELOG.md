...
- **employee-standards.md 新增 §7「浏览器验证证据链」**. 定义「什么算浏览器验证通过」的证据标准：改了用户可见可交互的东西就必须在浏览器里证明；自检不留测试代码但必须留验证记录（构建新鲜度 / 登录态真实性 / 期望锚定 spec / 控制台与网络无错）；截图不等于行为验证；有验证价值的浏览器路径必须写成 Playwright 测试、**提交前实跑留证（不依赖 CI）**；含断言的临时脚本必须同一 PR 内转正或删除。§5 数据编撰禁令同步扩展：验证期望值禁止编造、验证证据不得编造或复用旧证据。
