...
- 🔴 改 DOM/交互/跳转/异步渲染/样式/响应式，或守卫/权限/多角色可见性 → 浏览器验证（权限类必须真实登录态）；纯逻辑 → 按下方单测取舍，不开浏览器
...
- 🟡 分层反馈预期：单测改完随手跑（秒级量级）；验收测试提交前实跑（见下方交付，分钟级可接受）

**验收——对照 spec 验收标准的真实行为验证，层级由被验对象决定**
- 🔴 后端/服务 → 对真实运行的服务发真实请求验证契约与端到端行为，不以全 mock 的单测链冒充验收；验收落成集成测试，提交前实跑
- 🔴 前端/用户可见 → 浏览器自检 + Playwright 交付测试（见下方 A/B）
