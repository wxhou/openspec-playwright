...
- 🔴 后端/服务 → 对真实运行的服务发真实请求验证契约与端到端行为，不以全 mock 的单测链冒充验收，落成集成测试（真实数据/依赖，遵守 §4）；
- 🟡 所有验收期望锚定 spec/验收标准写「预期 X，实测 Y」（期望编造禁令见 §4）

**A. 自检（浏览器，开发中 AI 验证，不留测试代码）**
- 🔴 截图 ≠ 行为验证：交互必须验证结果（点击后的状态/跳转/渲染），仅截图不算通过；断言只作辅助
- 🟡 验证前核对加载的是本次产物（清缓存/停用 SW/核 hash）——跑旧包验证作废；权限类禁止注入 token，必须真实登录态，多角色各角色单独登录
...
- 🟡 无法覆盖的路径（skip/fixme）必附理由；稳定选择器（data-testid/role）；Healer 只消 flaky，不得掩盖断言失败
