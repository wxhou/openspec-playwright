...
- 🟡 需求理解不清或存在可见风险时，先停下来提问，不直接执行；动手前列假设 → 逐条验证，有不清再提问
- 🟡 多解释则全列，更简单方案则提出并坚持
...
- 🔴 改 DOM/交互/跳转/异步渲染/样式/响应式，或守卫/权限/多角色可见性 → 浏览器验证（权限类必须真实登录态）；纯逻辑 → 按 §7 测试策略取舍，不开浏览器
...
> WHY：AI 用琐碎单测堆数量冒充完成度——生成烧 token、套件拖慢反馈，验收结果才决定能否交付。测试的价值在验证行为，不在覆盖率数字。
...
- 🟡 分层反馈预期：单测改完随手跑（秒级量级）；验收测试提交前实跑（§6 B，分钟级可接受）

**验收——对照 spec 验收标准的真实行为验证，层级由被验对象决定**
- 🔴 后端/服务 → 对真实运行的服务发真实请求验证契约与端到端行为，不以全 mock 的单测链冒充验收
- 🔴 前端/用户可见 → 按 §6 浏览器验证 + Playwright 交付测试
- 🟡 集成测试用真实数据/依赖（seed/fixture，遵守 §4）；期望锚定 spec 写「预期 X，实测 Y」（同 §6 A），不自定期望
