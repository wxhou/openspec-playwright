...
**Spec idempotency**: If the .spec.ts exists → read it first. Add tests for requirements not yet covered; update tests whose source requirement changed; keep existing passing tests (confirm with the user before deleting any). Never wholesale-regenerate.

**MCP recording flow (conditional)**: if the `playwright-test` MCP server exposes `generator_setup_page` / `generator_write_test` / `generator_read_log`, prefer record-then-write over hand-writing: execute each step once in the real browser → `generator_write_test` emits the spec → review the generated file against the per-assertion check and selector rules below. Fall back to hand-writing when the tools are absent (they only exist on the test-runner MCP server).
