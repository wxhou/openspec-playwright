...
export async function migrate(options) {
    ...
    for (const item of items) {
        console.log(`  ${chalk.green("→")} ${item.fileName}  →  changes/${item.changeName}/${item.fileName}`);
    }
    // 4. Dry run
    if (options.dryRun) {
    ...
    console.log(chalk.blue("\n─── Next step ───"));
    console.log(chalk.green("  Run /opsx:e2e <name> (Claude Code) or /opsx-e2e <name> (OpenCode/Cline) to verify each migrated change.\n"));
}
