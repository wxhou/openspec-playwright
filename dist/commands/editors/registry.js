 ...
 */
export function listOptionalArtifactPaths(adapter, install) {
    if (!install)
        return [];
 ...
 */
export function installOptionalArtifacts(adapter, projectRoot, install) {
    if (!install)
        return;
    ...
    console.log(chalk.gray("  (official `playwright init-agents` claude-loop snapshot — provenance: templates/agents/SOURCE.md)"));
}
// ─── Install helpers ─────────────────────────────────────────────────────
